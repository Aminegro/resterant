/**
 * ═══════════════════════════════════════════════════════════════════════════
 * 🔥 ملك الطابون - Backend المركزي (العقل)
 * ═══════════════════════════════════════════════════════════════════════════
 * 
...
## هويتك
أنت مساعد طلبات مطعم "ملك الطابون" في العيزرية. تستقبل طلبات الزبائن باللهجة الفلسطينية.

## معلومات المطعم
- الاسم: ملك الطابون
- العنوان: العيزرية - دوار وادي النار
- ساعات العمل: 8:00 صباحاً - 2:00 بعد منتصف الليل (يومياً)
- وقت التحضير: 7-10 دقيقة
...
6. شرط القيام بالطلب جمع معلومات: الاسم + الطلب + المكان (داخل المحل أو بالسيارة)
7. قم بإخبار الزبون بالسعر مع طلب التأكيد
8. بعد التأكيد أخبره ان يسلتم طلبه بعد 7-10 دقائق
...
[ORDER_DATA]
{"customer":"اسم الزبون","items":"الأصناف المطلوبة","total":المبلغ_رقم,"location":"المكان"}
[/ORDER_DATA]

مثال:
[ORDER_DATA]
{"customer":"أحمد","items":"بيتزا الطابون×2, بيض مع جبنة×1","total":55,"location":"داخل المحل"}
[/ORDER_DATA]
...
app.post('/api/chat', chatLimiter, async (req, res) => {
    try {
        ...
        res.status(500).json({
            success: false,
            reply: 'عذراً، حصل خطأ. تواصل معنا على واتساب 0523668131',
            error: 'SERVER_ERROR'
        });
    }
});
...
/**
 * POST /api/orders
 * إنشاء طلب يدوي (للطلبات الهاتفية)
 */
app.post('/api/orders', ordersLimiter, (req, res) => {
    ...
    console.log(`📞 طلب يدوي جديد #${order.id}`);
