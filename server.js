/**
 * ═══════════════════════════════════════════════════════════════════════════
 * 🔥 ملك الطابون - Backend المركزي (العقل) - النسخة المطورة
 * ═══════════════════════════════════════════════════════════════════════════
 * 
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔐 الإعدادات الحساسة (من متغيرات البيئة)
// ═══════════════════════════════════════════════════════════════════════════

const OPENAI_API_KEY = process.env.OPENAI_API_KEY || '';

// تحذير إذا لم يتم ضبط المفتاح
if (!OPENAI_API_KEY) {
    console.warn('⚠️  تحذير: مفتاح OpenAI غير مُعد! أضفه في متغيرات البيئة');
}
...
const chatLimiter = rateLimit({
    ...
    max: 20, // 20 طلب كحد أقصى
    message: {
        success: false,
        error: 'الكثير من الطلبات! حاول مرة أخرى بعد دقيقة',
...
// ═══════════════════════════════════════════════════════════════════════════
// 🤖 System Prompt للذكاء الاصطناعي - النسخة المطورة
// ═══════════════════════════════════════════════════════════════════════════
...
6.أطلب منه الاسم + الطلب + المكان (داخل المحل أو بالسيارة أو توصيل مع العنوان)
...
10.حاول جمع كل المعلومات بأقل عدد رسائل ممكن.
11. مناطق التوصيل المتاحة: 134

## ✅ نظام أنواع الزبائن الثلاثة - مهم جداً!

### عند بدء المحادثة، اسأل الزبون:
"أهلاً وسهلاً! 🔥 شو بتحب تطلب؟ وبتحب تاخذه من وين؟ (داخل المحل / بالسيارة / توصيل)"
...
### مثال 1 - داخل المحل:
[ORDER_DATA]
{"customer":"محمد","phone":"","items":"بيتزا الطابون×2","total":40,"orderType":"dine_in","location":"داخل المحل","address":"","carInfo":"","deliveryNotes":""}
[/ORDER_DATA]

### مثال 2 - بالسيارة:
[ORDER_DATA]
{"customer":"أحمد","phone":"0599123456","items":"بيتزا تونا×1, بيض مع جبنة×1","total":40,"orderType":"car_pickup","location":"بالسيارة","address":"","carInfo":"سيارة بيضاء هيونداي","deliveryNotes":""}
[/ORDER_DATA]

### مثال 3 - توصيل:
[ORDER_DATA]
{"customer":"سارة","phone":"0569876543","items":"صواني كبير مع خضار×2","total":80,"orderType":"delivery","location":"العيزرية - حي الشمال","address":"شارع المدارس، بجانب مسجد النور، عمارة أبو علي الطابق 3","carInfo":"","deliveryNotes":"الباب الأخضر، رجاء الاتصال قبل الوصول"}
[/ORDER_DATA]
...
// ═══════════════════════════════════════════════════════════════════════════
// 🔌 API للذكاء الاصطناعي (للموقع الأول - العملاء)
// ═══════════════════════════════════════════════════════════════════════════

/**
 * POST /api/chat
 * يستقبل رسائل العملاء ويردها للذكاء الاصطناعي
 */
    ...
    try {
        ...
        if (!response.ok) {
            ...
            if (response.status === 429) {
                return res.status(429).json({
                    success: false,
                    reply: 'الخدمة مشغولة حالياً، حاول مرة أخرى',
                    error: 'RATE_LIMIT'
                });
            }
        ...
        if (orderMatch) {
            try {
                ...
                // ✅ إنشاء الطلب مع البيانات الجديدة
                const order = {
                    id: ++db.counter,
                    ...
                    orderType: orderData.orderType || 'dine_in', // نوع الطلب الجديد
                    location: orderData.location || 'غير محدد',
                    address: orderData.address || '', // العنوان التفصيلي للتوصيل
                    carInfo: orderData.carInfo || '', // معلومات السيارة
                    deliveryNotes: orderData.deliveryNotes || '', // ملاحظات التوصيل
                    status: 'new',
                ...
                // ✅ عرض تفاصيل مختلفة حسب نوع الطلب
                ...
                if (order.phone) console.log(`   📱 الجوال: ${order.phone}`);
                ...
                if (order.orderType === 'delivery') {
                    ...
                    if (order.deliveryNotes) console.log(`   📝 ملاحظات: ${order.deliveryNotes}`);
                }
                console.log(`${'═'.repeat(50)}\n`);

                // إزالة بيانات الطلب من الرد وإضافة رقم الطلب
...
// ═══════════════════════════════════════════════════════════════════════════
// 📦 API للطلبات (للموقع الثاني - العمال)
// ═══════════════════════════════════════════════════════════════════════════

/**
 * GET /api/orders
 * جلب كل الطلبات للعمال
 */
app.get('/api/orders', ordersLimiter, (req, res) => {
    autoCleanup();

    // ✅ فلترة حسب نوع الطلب (اختياري)
...
app.post('/api/orders', ordersLimiter, (req, res) => {
    ...
    if (!customerName || !items) {
        return res.status(400).json({
            success: false,
            error: 'اسم العميل والطلب مطلوبان'
        });
    }
...
/**
 * GET /api/orders/:id
 * جلب طلب واحد (للعميل لمتابعة طلبه)
 */
    ...
    if (status) {
        ...
        if (status === 'ready' && previousStatus !== 'ready') {
            ...
            order.readyNotification = {
                sent: true,
                message: `🎉 تم تجهيز طلبك #${order.id}! ${orderTypeMsg[order.orderType] || ''}`,
...
/**
 * GET /api/orders/poll
 * Long polling للتحديثات الفورية
 */
...
╔═══════════════════════════════════════════════════════════════════════════╗
║                                                                           ║
║   🔥  ملك الطابون - Backend المركزي v3.0                                 ║
║                                                                           ║
